...
**Architecture:** Swift Package Manager 기반. 테스트 가능한 순수 로직은 `ClaudeBarCore` 라이브러리 타깃에, AppKit/SwiftUI UI는 `claude-bar` 실행 타깃에 분리한다. 데이터 소스는 (1) macOS 키체인의 Claude Code OAuth 토큰으로 호출하는 사용량 API, (2) `~/.claude/projects/**/*.jsonl` 로컬 파싱 두 가지.
...
- Create: `Sources/ClaudeBarCore/Version.swift`
- Create: `Sources/ClaudeBar/main.swift`
- Create: `.gitignore`

**Interfaces:**
- Produces: `ClaudeBarCore` 라이브러리 타깃(이후 모든 로직이 여기 추가됨), `claude-bar` 실행 타깃, `ClaudeBarCoreTests` 테스트 타깃.
...
`Sources/ClaudeBarCore/Version.swift`:
```swift
...
print("ClaudeBar \(ClaudeBar.version)")
```
...
Run: `cd ~/Workspace/claude-bar && swift build && swift run claude-bar`
Expected: `ClaudeBar 0.1.0` 출력
...
**Files:**
- Create: `Sources/ClaudeBarCore/Models.swift`
- Test: `Tests/ClaudeBarCoreTests/ModelsTests.swift`
...
- Produces:
  ...
  - `enum ClaudeBarError: Error { case keychainUnavailable(OSStatus), credentialsMalformed, apiError(Int) }`

- [ ] **Step 1: 실패하는 테스트 작성**

`Tests/ClaudeBarCoreTests/ModelsTests.swift`:
```swift
import XCTest
...
`Sources/ClaudeBarCore/Models.swift`:
```swift
import Foundation
...
**Files:**
- Create: `Sources/ClaudeBarCore/UsageResponse.swift`
- Test: `Tests/ClaudeBarCoreTests/UsageResponseTests.swift`
...
`Tests/ClaudeBarCoreTests/UsageResponseTests.swift`:
```swift
import XCTest
@testable import ClaudeBarCore
...
`Sources/ClaudeBarCore/UsageResponse.swift`:
```swift
import Foundation
...
- Test: `Tests/ClaudeBarCoreTests/CredentialsTests.swift`

**Interfaces:**
- Consumes: `UsageResponse` (Task 3), `ClaudeBarError` (Task 2)
- Produces:
...
`Tests/ClaudeBarCoreTests/CredentialsTests.swift`:
```swift
import XCTest
@testable import ClaudeBarCore
...
**Files:**
- Create: `Sources/ClaudeBarCore/Pricing.swift`
- Test: `Tests/ClaudeBarCoreTests/PricingTests.swift`
...
`Tests/ClaudeBarCoreTests/PricingTests.swift`:
```swift
import XCTest
@testable import ClaudeBarCore
...
`Sources/ClaudeBarCore/Pricing.swift`:
```swift
import Foundation
...
**Files:**
- Create: `Sources/ClaudeBarCore/TranscriptScanner.swift`
- Test: `Tests/ClaudeBarCoreTests/TranscriptScannerTests.swift`
...
`Tests/ClaudeBarCoreTests/TranscriptScannerTests.swift`:
```swift
import XCTest
@testable import ClaudeBarCore
...
`Sources/ClaudeBarCore/TranscriptScanner.swift`:
```swift
import Foundation
...
Run: 임시 스크립트 없이 `swift test` 전체 통과 확인 후, 다음 한 줄 확인용 실행 파일 코드를 `Sources/ClaudeBar/main.swift`에 잠깐 추가하지 말고, `swift repl` 대신 이렇게 확인:

```bash
swift run claude-bar  # 아직 버전 출력만 함 — 실데이터 검증은 Task 9 앱 조립 후 팝오버에서 확인
```
...
**Files:**
- Create: `Sources/ClaudeBarCore/NotificationPlanner.swift`
- Test: `Tests/ClaudeBarCoreTests/NotificationPlannerTests.swift`
...
`Tests/ClaudeBarCoreTests/NotificationPlannerTests.swift`:
```swift
import XCTest
@testable import ClaudeBarCore
...
`Sources/ClaudeBarCore/NotificationPlanner.swift`:
```swift
import Foundation
...
**Files:**
- Create: `Sources/ClaudeBarCore/CharacterSprites.swift`
- Test: `Tests/ClaudeBarCoreTests/CharacterSpritesTests.swift`
...
`Tests/ClaudeBarCoreTests/CharacterSpritesTests.swift`:
```swift
import XCTest
@testable import ClaudeBarCore
...
`Sources/ClaudeBarCore/CharacterSprites.swift`:
```swift
import Foundation
...
**Files:**
- Create: `Sources/ClaudeBar/SpriteRenderer.swift`
- Create: `Sources/ClaudeBar/AppState.swift`
- Create: `Sources/ClaudeBar/StatusItemController.swift`
- Modify: `Sources/ClaudeBar/main.swift` (전체 교체)
...
`Sources/ClaudeBar/SpriteRenderer.swift`:
```swift
import AppKit
import ClaudeBarCore
...
`Sources/ClaudeBar/AppState.swift`:
```swift
import AppKit
import Combine
import ClaudeBarCore
...
`Sources/ClaudeBar/StatusItemController.swift`:
```swift
import AppKit
import Combine
import ClaudeBarCore
...
`Sources/ClaudeBar/main.swift` (전체 교체):
```swift
import AppKit
import ClaudeBarCore
...
Run: `swift build && swift test && swift run claude-bar` (백그라운드 실행 후 메뉴바 확인, 확인 끝나면 Ctrl+C)
Expected:
...
**Files:**
- Create: `Sources/ClaudeBar/PopoverView.swift`
- Create: `Sources/ClaudeBar/Formatters.swift`
- Modify: `Sources/ClaudeBar/main.swift` (팝오버 연결)
...
`Sources/ClaudeBar/Formatters.swift`:
```swift
import Foundation
...
주의: `LaunchAtLogin`은 Task 11에서 만들므로 이 시점에는 컴파일 에러가 난다. Task 10과 11을 연달아 진행하거나, 이 시점에 임시 스텁을 두려면 `Sources/ClaudeBar/LaunchAtLogin.swift`에 아래 최소 구현을 먼저 넣는다(Task 11에서 완성):
```swift
enum LaunchAtLogin {
...
`Sources/ClaudeBar/main.swift`의 `AppDelegate`를 다음으로 교체:
```swift
import AppKit
import SwiftUI
import ClaudeBarCore
...
Run: `swift build && swift run claude-bar`
Expected:
...
**Files:**
- Create: `Sources/ClaudeBar/NotificationManager.swift`
- Create(또는 스텁 교체): `Sources/ClaudeBar/LaunchAtLogin.swift`
- Modify: `Sources/ClaudeBar/main.swift` (알림 연결)
...
`Sources/ClaudeBar/NotificationManager.swift`:
```swift
import Foundation
...
`Sources/ClaudeBar/LaunchAtLogin.swift`:
```swift
import Foundation
...
**Interfaces:**
- Produces: `make bundle` → 저장소 루트에 `ClaudeBar.app` 생성 (ad-hoc 서명). Homebrew formula(Task 14)가 이 타깃을 사용.
...
<dict>
    ...
    <string>com.claude-bar.ClaudeBar</string>
    <key>CFBundleName</key>
    <string>ClaudeBar</string>
    <key>CFBundleExecutable</key>
    <string>claude-bar</string>
...
2. 팝오버의 "로그인 시 자동 시작" 토글이 활성화되어 있고, 켜면 시스템 설정 > 로그인 항목에 ClaudeBar가 나타난다
...
`LICENSE`: MIT 라이선스 전문. 저작권 줄은 `Copyright (c) 2026 ClaudeBar contributors`.
...
### Homebrew (recommended)
    ...
    brew tap <GITHUB_USER>/claude-bar https://github.com/<GITHUB_USER>/claude-bar
    brew install claude-bar
    claude-bar &

### From source

    git clone https://github.com/<GITHUB_USER>/claude-bar && cd claude-bar
    make run
...
`Formula/claude-bar.rb` (`GITHUB_USER`는 Step 1의 계정명으로 치환):
```ruby
class ClaudeBar < Formula
  desc "Cute pixel-art menu bar widget showing Claude Code usage"
  homepage "https://github.com/GITHUB_USER/claude-bar"
  url "https://github.com/GITHUB_USER/claude-bar/archive/refs/tags/v0.1.0.tar.gz"
...
# 사용자가 GitHub에 claude-bar 공개 저장소 생성 후:
git remote add origin https://github.com/GITHUB_USER/claude-bar.git
...
curl -sL https://github.com/GITHUB_USER/claude-bar/archive/refs/tags/v0.1.0.tar.gz | shasum -a 256
# Formula/claude-bar.rb의 sha256 라인을 실제 값으로 교체 후:
git add Formula/claude-bar.rb && git commit -m "chore: pin formula sha256 for v0.1.0" && git push
```
...
claude-bar &
```
Expected: 소스 빌드 후 메뉴바에 ClaudeBar가 뜬다. Gatekeeper 경고 없음(로컬 빌드).
